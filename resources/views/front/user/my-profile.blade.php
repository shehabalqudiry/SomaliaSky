...
@section('content')
    ...
    <div class="container my-3">
        <a href="{{ route('front.announcements.create') }}" class="btn btn-primary my-3"><span class="fal fa-plus"></span>
            {{ __('lang.Add') }}</a>
        <a href="{{ route('front.profile.edit') }}" class="btn btn-primary my-3"><span class="fal fa-edit"></span>
        ...
        <div class="row text-center">
            @forelse($user->announcements->where('type', 0) as $anncment)
                <div class="col-6 col-md-4 col-lg-2">
                    {{-- @if ($anncment->store_id == null) --}}
                    <a href="{{ route('front.announcements.show', $anncment) }}" class="mx-2 rounded-3 mb-4 h-hv-50">
                    ...
                    <h6>{{ $anncment->title }}
                        <a href="#" onclick="document.getElementById('form_{{ $anncment->number }}').submit()"><i
                                class="fas fa-trash"></i></a>
                        <form id="form_{{ $anncment->number }}"
                            action="{{ route('front.announcements.destroy', $anncment) }}" method="POST" class="d-none">
                            @csrf
                            @method('DELETE')
